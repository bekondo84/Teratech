...
import com.megatim.common.utilities.OrderByItem;
...
public class ValidateAndFillBeans {
    ...
    public static void cleanBeanField(Object _source) throws IllegalArgumentException, IllegalAccessException {
        ...
        for (Field _field : _fields) {
            ...
            } else if (_value instanceof JCalendar) {
                ((JCalendar) _value).setDate(new Date());
            } else if (_value instanceof JDateChooser) {
                ((JDateChooser) _value).setDate(new Date());
            } else if (_value instanceof JTable) {
                //Recuperation du model
                AbstractTableBaseListModel model = (AbstractTableBaseListModel) ((JTable) _value).getModel();
                //Vider le model
                List rows = model.getElements();
                //Suppression de l'"element
                for (int i = 0; i < rows.size(); i++) {
                    model.deleteElement(i);
                }
            } else if (_value instanceof JCheckBox) {
                ((JCheckBox) _value).setSelected(false);
            } else if (_value instanceof AbstractTablePanel) {

                //Recuperation du model
                AbstractTableBaseListModel model = (AbstractTableBaseListModel) ((AbstractTablePanel) _value).getModel();
                //Vider le model
                List rows = model.getElements();
                //Suppression de l'"element
                for (int i = 0; i < rows.size(); i++) {
            ...
            }else if(_value instanceof AbstractTreePanel){
                
                //Recuperation du modele
                AbstractTreeBaseListModel model = ((AbstractTreePanel)_value).getModel();
                List rows = model.getElements();
                for(int i = 0 ; i<rows.size();i++){
                    model.deleteElement(i);
                }
    ...
    private static void setIHMFieldValue(Field _cibleField, Object _cible, Field _sourceField, Object _source) throws IllegalArgumentException, IllegalAccessException {
       //System.out.println("ValidateAndFillBean.setIHMFieldValue() ::: "+_cibleField+" :::: "+_cible+" ::: "+_sourceField+" ::: "+_source);
        ...
        } else if (_cibleField.getType().equals(JDateChooser.class)) {
            ((JDateChooser) _cibleField.get(_cible)).setDate((Date) value);
        ...
        } else if (_cibleField.getType().equals(CustomComboBox.class)) {
            //System.out.println("ValidateAndFillBeans.setIHMFieldValue(Field _cibleField, Object _cible, Field _sourceField, Object _source) ::::::::::::::::::::::::::::::::: "+value);
            if (value != null) {
                ((CustomComboBox) _cibleField.get(_cible)).setSelectedItem(value);
            }
        } else if (_cibleField.getType().equals(JTable.class)) {
            //Cas ou les donn�es sont stock� dans des liste         
            JTable table = ((JTable) _cibleField.get(_cible));
            AbstractTableBaseListModel model = (AbstractTableBaseListModel) table.getModel();
        ...
        } else if (_cibleField.getType().equals(AbstractTablePanel.class)) {
            //Cas ou les donn�es sont stock� dans des liste         
            AbstractTablePanel table = ((AbstractTablePanel) _cibleField.get(_cible));
            AbstractTableBaseListModel model = (AbstractTableBaseListModel) table.getModel();
            if (value instanceof List) {
                List rows = ((List) value);
                if (rows != null && !rows.isEmpty()) {
        ...
        }else if(_cibleField.getType().equals(AbstractTreePanel.class)){
            AbstractTreePanel tree = (AbstractTreePanel) _cibleField.get(_cible);
            AbstractTreeBaseListModel model = tree.getModel();
            if (value instanceof List) {
                List rows = ((List) value);
                if (rows != null && !rows.isEmpty()) {
    ...
    private static synchronized boolean validateField(Object _bean, Field _field) throws IllegalArgumentException, IllegalAccessException {
        ...
        if (!annot.nullable()) {
            ...
            if (value == null) {
                ...
                errorFieldsMapInitialised(_bean, annot.errorMessageField(), annot.errorMessage());
                _validate &= false;
            } else if ((value instanceof JTable) && ((AbstractTableBaseListModel) value).getRowCount() <= 0) {
                errorFieldsMapInitialised(_bean, annot.errorMessageField(), annot.errorMessage());
                _validate &= false;
            }
    ...
    public static void updatableAnnotationProcessor(Object _source) throws IllegalArgumentException, IllegalAccessException {
        ...
        for (Field _field : _fields) {
            ...
            if (_field.isAnnotationPresent(Champ.class)) {
                ...
                if (!annot.update()) {
                    _field.setAccessible(true);
                    Object value = _field.get(_source);

                    if (value instanceof JTextComponent) {
                       // ((JTextComponent) value).setEditable(false);
                    	CommonsUtilities.buildJTextFielddisable(((JTextComponent) value));
                    } else if (value instanceof JComboBox) {
                       // ((JComboBox) value).setEnabled(false);
                    	 CommonsUtilities.buildJComboboxdisable((JComboBox) value);
                    } else if (value instanceof JDateChooser) {
                       // ((JDateChooser) value).setEnabled(false);
                    	 CommonsUtilities.buildJDatedisable((JDateChooser) value);
                    } else if (value instanceof JRadioButton) {
                        ((JRadioButton) value).setEnabled(false);
                    } else if (value instanceof JCheckBox) {
                        ((JCheckBox) value).setEnabled(false);
                    }
                }
            }
        }
    ...
    public static void disableAllFields(Object _source) throws IllegalArgumentException, IllegalAccessException {
        ...
        for (Field _field : _fields) {
            ...
            if (value instanceof JTextComponent) {
            	CommonsUtilities.buildJTextFielddisable(((JTextComponent) value));
               // ((JTextComponent) value).setEnabled(false);
            } else if (value instanceof JComboBox) {
               // ((JComboBox) value).setEnabled(false);
                CommonsUtilities.buildJComboboxdisable((JComboBox) value);
            } else if (value instanceof JDateChooser) {
               // ((JDateChooser) value).setEnabled(false);
                CommonsUtilities.buildJDatedisable((JDateChooser) value);
            } else if (value instanceof JFormattedTextField) {
            	CommonsUtilities.buildJTextFielddisable((JFormattedTextField) value);
                //((JFormattedTextField) value).setEnabled(false);
            } else if (value instanceof JRadioButton) {
                ((JRadioButton) value).setEnabled(false);
            } else if (value instanceof JCheckBox) {
                ((JCheckBox) value).setEnabled(false);
            }else if( value  instanceof AbstractTablePanel){
                AbstractTablePanel panel = (AbstractTablePanel) value ;
                panel.enableAddButton(false);panel.enableDeleteButton(false);
                panel.enableEditButton(false);
            }else if( value  instanceof AbstractTreePanel){
                AbstractTreePanel panel = (AbstractTreePanel) value ;
                panel.enableAddButton(false);panel.enableDeleteButton(false);
                panel.enableEditButton(false);
            }
        }
    ...
    public static void errorFieldsMapInitialised(Object _instance, String errorMessageField,
        ...
        for (Field _field : fields) {
            ...
            //System.out.println("Je suis dans ValidateField  "+_field.getName());
            //Verification nom du champs
            if (_field.getName().equals(errorMessageField)) {
        ...
        //Activation de la
        if (chechField.get(_instance).getClass().equals(JLabel.class)) {
            ((JLabel) chechField.get(_instance)).setForeground(Color.red);
            ((JLabel) chechField.get(_instance)).setText(MessagesBundle.getMessage(errorMesssage));
        }
        return;
    }
    ...
    private static Object getFieldValue(Object _bean, Field _field) throws IllegalArgumentException, IllegalAccessException, NumberFormatException {
        ...
        //Section composant swing
        Object _value = _field.get(_bean);
        if (_value != null) {
            if (_field.getType().equals(JTextField.class)) {
                value = getWrapObject(annot.type(), ((JTextField) _field.get(_bean)).getText());
            } else if (_field.getType().equals(JFormattedTextField.class)) {
                String fieldValue = "" + ((JFormattedTextField) _field.get(_bean)).getValue();
                //System.out.println("ValidateAndFillBeans.getFieldValue():::::::::::::::::: "+fieldValue.trim());
                value = getWrapObject(annot.type(), fieldValue.trim());
                //System.out.println(annot.type()+"::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: "+value);
            } else if (_field.getType().equals(JDateChooser.class)) {

                value = ((JDateChooser) _field.get(_bean)).getDate();

            } else if (_field.getType().equals(JCheckBox.class)) {

                value = ((JCheckBox) _field.get(_bean)).isSelected();

            } else if (_field.getType().equals(JRadioButton.class)) {

                value = ((JRadioButton) _field.get(_bean)).isSelected();

            } else if (_field.getType().equals(JSpinner.class)) {

                value = ((JSpinner) _field.get(_bean)).getValue();

            } else if (_field.getType().equals(JLabel.class)) {
                value = getWrapObject(annot.type(), ((JLabel) _field.get(_bean)).getText());
            } else if (_field.getType().equals(JPasswordField.class)) {
                value = getWrapObject(annot.type(), ((JPasswordField) _field.get(_bean)).getText());
                //value = DESEncrypter.getInstance().encryptText(""+value_1);
                //System.out.println("ValidateAndFillBeans.getFieldValue():::::::::::::::::: "+value_1+" :::::::::::: "+value);
            } else if (_field.getType().equals(JTextArea.class)) {
                value = getWrapObject(annot.type(), ((JTextArea) _field.get(_bean)).getText());
            } else if (_field.getType().equals(JComboBox.class)) {
                Object selectValue = ((JComboBox) _field.get(_bean)).getSelectedItem();

                if (selectValue != null) {
                    if (selectValue instanceof String) {
                        value = getWrapObject(annot.type(), (String) selectValue);
                        if(value!=null&&value.getClass().equals(annot.type())){
                            return value ;
                        }
                        return null;
                    } else {
            ...
            } else if (_field.getType().equals(CustomComboBox.class)) {
                Object selectValue = ((CustomComboBox) _field.get(_bean)).getSelectedItem();
                ...
                //System.out.println(" KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK::::::::::::::::::::::::::::::::::::::::::::: "+value);
            } else if (_field.getType().equals(JTable.class)) {
                //Table
                JTable table = (JTable) _field.get(_bean);

                AbstractTableBaseListModel model = (AbstractTableBaseListModel) table.getModel();

                return model.getElements();
            }  else if (_field.getType().equals(JList.class)) {
                //Table
                JList liste = (JList) _field.get(_bean);               
                List results = new ArrayList();
                for(int i=0 ; i<liste.getModel().getSize();i++){
                    results.add(liste.getModel().getElementAt(i));
                }
                return results;
            } else if (_field.getType().equals(AbstractTablePanel.class)) {

                //Table
                AbstractTablePanel table = (AbstractTablePanel) _field.get(_bean);

                AbstractTableBaseListModel model = (AbstractTableBaseListModel) table.getModel();

                return model.getElements();
            }else if(_field.getType().equals(AbstractTreePanel.class)){
                AbstractTreePanel tree = (AbstractTreePanel) _field.get(_bean);
                AbstractTreeBaseListModel model = tree.getModel();
                return model.getElements();
            }
        }
    ...
    private static Object getFieldValue2(Object _bean, Field _field) throws IllegalArgumentException, IllegalAccessException, NumberFormatException {
        ...
        if (_value != null) {
            if (_field.getType().equals(JTextField.class)) {
                ...
                value = getWrapObject(annot.type(), "" + ((JFormattedTextField) _field.get(_bean)).getValue());
            } else if (_field.getType().equals(JDateChooser.class)) {
                value = ((JDateChooser) _field.get(_bean)).getDate();
            } else if (_field.getType().equals(JCheckBox.class)) {
                value = ((JCheckBox) _field.get(_bean)).isSelected();
                ...
                if (selectValue != null) {
                    //System.out.println("ValidateAndAndFillBeans.getWrapObject(Class c , String value) ::: "+selectValue+" :::: "+selectValue.getClass());
                ...
                AbstractTreeBaseListModel model = tree.getModel();
                return model.getElements();                
            }else if(_field.getType().equals(Comparable.class)){
